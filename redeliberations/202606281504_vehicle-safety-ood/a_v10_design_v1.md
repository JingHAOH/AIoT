...
**类型形态**：`struct`（值对象）。计数器由两个不可变整数值定义相等性，无独立标识。Trip 聚合持有本计数器，基础设施层在检测到急刹/急加速事件时以"替换为新计数器"的方式更新 Trip 聚合中的该值对象（不可变值对象的更新模式）。
...
**类型形态**：`struct`（值对象）。Tracker 的属性值定义相等性，无独立标识。L3 持续时长的推进表现为以"新 Tracker 实例替换旧实例"的方式更新——PermissionService 接收每个 RiskDeterminedEvent / RiskResolvedEvent 后，读取 Trip 聚合中当前 Tracker，计算更新后的 Tracker 并写回 Trip 聚合（不可变值对象的更新模式）。
...
**类型形态**：`struct`（值对象）。通知偏好由一组风险等级集合的值定义相等性，不可变。家属修改偏好配置意味着创建新的 NotificationPreference 实例替换旧实例。
...
**类型形态**：`struct`（值对象）。升级状态由阶段标识、目标版本、已传输偏移量和阶段时间戳的组合值定义相等性，无独立标识、不可变。状态推进表现为以新实例替换旧实例。其概念维度包括：
...
**类型形态**：`struct`（值对象）。DetectionWindow 由窗口各属性的组合值定义相等性，无独立标识、不可变。窗口状态的推进表现为以"新 DetectionWindow 实例替换旧实例"的方式。其概念维度包括：
...
**类型形态**：`struct`（值对象）。OverrideSignal 由操作类型和时间戳的组合值定义相等性，无独立标识、不可变。其概念维度包括：
...
**类型形态**：`struct`（值对象）。DriverComprehensiveScore 由评分值定义相等性，无独立标识、不可变。Driver 综合风险评分的更新以"新 DriverComprehensiveScore 实例替换 Driver 聚合中的旧实例"方式完成，不可变性使评分更新的事务边界清晰。构造时须确保值在 [0,100] 范围内。
...
**类型形态**：`struct`（值对象）。RescueAuthorizationToken 由凭证标识、签发机构、目标 Vehicle/Driver 标识、授权操作集合（远程解锁/健康档案调取）、签发时间戳和有效期组合定义相等性，无独立标识、不可变。校验通过后凭证被标记为已消费——以新的已消费 RescueAuthorizationToken 实例替换旧实例。
...
领域事件总线和各消费方依赖一个统一的公共抽象来确定"一个类型如何才能被视为领域事件"。本设计将 `DomainEvent` 定义为一个**仓颉 `interface`（marker interface）**，所有领域事件类型在实现时实现该接口。

为消除决策 8（聚合标识使用仓颉的 `struct` 类型）与 aggregateId 使用 `String` 类型之间的不一致，将 `aggregateId` 从裸 `String` 改为带类型标记的 `AggregateId` struct——该 struct 封装聚合根标识的字面值及其聚合类型枚举，使事件消费方可在编译期区分不同聚合根标识类型，避免手动解析字符串：

```
struct AggregateId {
    let value: String
    let aggregateType: AggregateType  // enum: TRIP | DRIVER | VEHICLE | SYSTEM_ACCOUNT | ROAD_RAGE_VOICE_RECORD
}
```

其中 `AggregateType` 为 `enum`，穷举系统中五类聚合根类型。该 struct 为值类型（值相等语义），不可变——与决策 8"聚合标识使用仓颉的 struct 类型"一致。
...
- **`aggregateId`**：关联的聚合根标识，以 `AggregateId` struct 表达，标识此事件源于哪个聚合根的状态变更。`AggregateId.value` 承载标识字面值（如 TripId、DriverId 等），`AggregateId.aggregateType` 提供编译期可区分的聚合根类型标签，使事件消费方无需手动解析字符串即可在编译期区分聚合根标识类型。

各具体领域事件类型（如 `RiskDeterminedEvent`、`FamilyAccessGrantedEvent` 等）在仓颉中以 `struct` 定义并实现 `DomainEvent` 接口——`struct` 保证事件载荷的不可变性，`implements DomainEvent` 保证事件总线可在编译期校验事件类型的有效性。`eventId`、`occurredAt`、`aggregateId` 这三个元数据字段由各事件 struct 自行携带（仓颉 `interface` 可声明属性要求），供事件总线提取后写入 outbox 表或在日志/审计中使用，领域消费方可通过事件实例直接访问这些公共元数据而不必依赖 outbox 表。
...
仓颉标准库未原生提供领域事件发布/订阅机制，需自定义实现。为解除所有事件发布/订阅代码对具体实现细节的耦合，本节定义领域事件总线的接口契约。
...
```
func publish<E: DomainEvent>(event: E): Result<Unit, EventPublishError>
```
...
```
func registerSyncHandler<E: DomainEvent>(eventTypeName: String, handler: (E) -> Unit)
```

- **`eventTypeName`**：以字符串表达的事件类型判别符（如 `"RiskDeterminedEvent"`）。选择字符串而非 `Type<E>` 作为事件路由键的理由：仓颉语言对运行时类型反射/类型标记（`Type<E>`）的支持程度需在实现阶段确认——若仓颉不原生支持运行时类型反射，`Type<E>` 无法在运行时用于消费方路由。以字符串作为路由键是语言无关的替代方案，确保事件总线接口契约在仓颉实现时不受类型反射能力制约。**实现阶段若确认仓颉支持运行时类型反射，可优化为类型安全注册 `registerSyncHandler<E: DomainEvent>(eventType: Type<E>, handler: (E) -> Unit)`**，此时 `eventTypeName` 可由 `Type<E>` 在编译期推导，提供更强的类型安全性。
...
```
func registerAsyncHandler<E: DomainEvent>(eventTypeName: String, handler: (E) -> Unit): Subscription
```

- **`eventTypeName`**：与同步消费注册一致（契约 2），以字符串表达事件类型判别符。同理保留类型安全注册的优化空间——实现阶段若仓颉支持运行时类型反射，可优化为 `registerAsyncHandler<E: DomainEvent>(eventType: Type<E>, handler: (E) -> Unit): Subscription`。
...
领域服务与 AlertPersistenceService 等多处依赖仓储接口进行聚合根的持久化和加载操作。为统一契约标准，本节定义四大聚合根仓储的核心接口契约。仓储接口以仓颉 `interface` 声明于领域层（如 `domain.model` 模块或独立 `domain.repository` 模块），由基础设施层提供具体实现并在装配阶段注入各领域服务。
...
```
func findById(tripId: TripId): Result<Option<Trip>, PersistenceError>
func save(trip: Trip): Result<Unit, PersistenceError>
```

- **findById**：按行程标识加载 Trip 聚合。返回值使用 `Result<Option<Trip>, PersistenceError>`——`Option.None` 表达"行程不存在"的正常业务情形（非错误），`PersistenceError` 表达数据源不可用等系统级故障。
- **save**：持久化 Trip 聚合（创建或更新）。通过 Trip 聚合内部的乐观锁版本号检测并发写冲突——若保存时发现版本号与数据源不一致（已被其他操作更新），返回 `PersistenceError.OptimisticLockConflict`。调用方（领域服务）据此决定重试或返回业务错误。
- **PersistenceError**：至少表达 `ConnectionFailed`（数据源不可用）、`OptimisticLockConflict`（乐观锁版本冲突）、`UnknownError`（未分类的持久化故障）。领域服务对乐观锁冲突按场景分别处理——评分计算等非实时场景可重试，安全判定等实时场景记录错误并降级。
...
```
func findById(driverId: DriverId): Result<Option<Driver>, PersistenceError>
func save(driver: Driver): Result<Unit, PersistenceError>
func updateScore(driverId: DriverId, newScore: DriverComprehensiveScore): Result<Unit, PersistenceError>
```

- **findById**：按驾驶员标识加载 Driver 聚合。语义同 TripRepository.findById。
- **save**：持久化 Driver 聚合。乐观锁冲突处理同 TripRepository.save。
- **updateScore**：轻量更新方法，仅更新 Driver 综合风险评分字段（DriverScoreUpdateService 使用）。以原子条件更新实现乐观锁——`WHERE version = expectedVersion`，冲突时返回 `OptimisticLockConflict`。与 `findById + save` 模式等价但性能更优（避免全量加载 → 修改 → 写回），适用于 DS-18 仅需更新单一字段的场景。
...
```
func findById(accountId: AccountId): Result<Option<SystemAccount>, PersistenceError>
func findByDriver(driverId: DriverId): Result<Array<SystemAccount>, PersistenceError>
func save(account: SystemAccount): Result<Unit, PersistenceError>
```

- **findByDriver**：按驾驶员的 Driver 标识查询与该驾驶员存在监护关系的所有家属账户（AccountRole=FAMILY）。返回结果为空数组时表达"无关联家属"的正常业务情形。供 PermissionService 在权限授予/撤销时查找关联家属。
- **save**：乐观锁冲突处理与其他仓储一致。
...
```
func findById(recordId: RecordId): Result<Option<RoadRageVoiceRecord>, PersistenceError>
func findByAlertId(alertId: AlertId): Result<Option<RoadRageVoiceRecord>, PersistenceError>
func findByExpiryBefore(deadline: Timestamp): Result<Array<RoadRageVoiceRecord>, PersistenceError>
func save(record: RoadRageVoiceRecord): Result<Unit, PersistenceError>
func delete(recordId: RecordId): Result<Unit, PersistenceError>
```
...
- **delete**：物理删除过期的存证记录。删除失败时返回 `PersistenceError`，由 PrivacyProtectionService 降级为标记状态并等待下一轮清除。
...
- **findById**：按车辆标识加载 Vehicle 聚合。返回 `Result<Option<Vehicle>, PersistenceError>`——`Option.None` 表达"车辆不存在"的正常业务情形，`PersistenceError` 表达数据源不可用等系统级故障。供 SensorSelfCheckService（DS-14）读取和更新传感器自检状态、OTAUpdateService（DS-15）读取当前固件版本和 OTA 升级状态、EmergencyRescueService（DS-12）更新车门锁状态。
- **save**：持久化 Vehicle 聚合（创建或更新）。乐观锁冲突检测与其他仓储一致——通过 Vehicle 聚合内部的乐观锁版本号检测并发写冲突（如 OTA 升级状态更新与传感器自检状态更新同时对 Vehicle 聚合执行写操作），冲突时返回 `PersistenceError.OptimisticLockConflict`，调用方按场景分别处理：传感器自检立即重试一次（自检为周期性操作），OTA 升级在重试上限内重试（见 §6.3）。
- **PersistenceError**：语义同 TripRepository 中的 PersistenceError 定义——`ConnectionFailed`、`OptimisticLockConflict`、`UnknownError`。

#### 3.7.6 仓储设计原则

- **接口在领域层、实现在基础设施层**：所有仓储接口以仓颉 `interface` 声明于领域层，基础设施层提供基于具体存储引擎（GaussDB/RDS/边缘侧本地存储）的实现并在装配阶段注入。
- **乐观锁版本号对领域层透明**：领域服务通过 `PersistenceError.OptimisticLockConflict` 感知冲突，不直接接触版本号字段。版本号由仓储实现在 save 时管理。
- **返回类型统一使用 Result 封装**：以 `Result<T, PersistenceError>` 将正常的"实体不存在"（`Option.None` / 空数组）与系统级持久化故障明确区分，消费方据此决定降级策略（§5.3）。
...
**仓颉语言考量**：DrivingBehaviorTrackingPort 以仓颉 `interface` 声明于领域层；DrivingBehaviorCounters 为 `struct` 值对象；DS-17 保持无状态、无持久字段（服务的计数器更新表现为接收事件→更新 Trip 聚合，通过仓储持久化），与现有 DS-14 SensorSelfCheckService 模式一致。
...
核心方法契约：
```
func pushNotification(recipient: AccountId, payload: NotificationPayload): Result<Unit, NotificationError>
```
- `NotificationPayload` 封装通知的内容类型（告警 / 救援报告 / 状态快照 / 绩效预警）、标题、正文和优先级。
- `NotificationError` 至少表达 `DeliveryFailed`（SMN 投递失败，基础设施层重试或降级）和 `RecipientUnreachable`（接收方不可达，如未安装 APP 或未建立长连接）。
...
核心方法契约：
```
func deliverRescueReport(report: RescueReport): Result<Unit, RescueReportError>
```
- `RescueReportError` 至少表达 `DeliveryFailed`（链路故障）和 `AckTimeout`（救援中心未在约定时间内确认接收）。DS-12 据此决定是否降级（如改用备用通信通道、或标记报告为"待补发"）。
...
核心方法契约：
```
func establishSession(participants: (SystemAccount, Driver), sessionType: SessionType): Result<SessionHandle, MediaSessionError>
func terminateSession(handle: SessionHandle): Result<Unit, MediaSessionError>
```
...
- `MediaSessionError` 至少表达 `SessionEstablishFailed`（信令或媒体通道建立失败）和 `SessionNotFound`（终止时未找到对应会话）。会话建立失败的处理策略由领域层在调用方决定（如 DS-08 在权限授予后尝试建立会话，失败则返回特定错误），不抛异常。
...
核心方法契约：
```
func deliverPackage(vehicle: VehicleId, package: OTAPackage, resumeFrom: Option<ByteOffset>): Result<DeliveryProgress, OTADeliveryError>
```
- `OTAPackage` 封装升级包摘要（版本、大小、校验和、分片元信息），领域层不关心物理文件存储位置。
- `resumeFrom` 可选参数标识断点续传的起始偏移字节——`None` 表示全新下发，`Some(offset)` 表示从已传输偏移量继续。
- `DeliveryProgress` 封装当前传输进度（已传输字节、总字节、是否完成），供 DS-15 驱动状态机转换。
- `OTADeliveryError` 至少表达 `TransmissionFailed`（传输中断，DS-15 据此进入重试/回滚分支）和 `ChecksumMismatch`（校验失败，DS-15 据此进入回滚分支）。

**仓颉语言考量**：四个端口均以仓颉 `interface` 声明于领域层（可置于 `domain.model` 模块或独立的 `domain.port` 模块），基础设施层提供具体实现并在装配阶段注入。`Result`、`Option`、`enum` 等类型构造均为前序已确认可行的仓颉标准类型，无新增语言能力诉求。
...
- **领域层通过端口参数声明阈值需求**：DrivingBehaviorTrackingPort 的方法契约以**事件回调风格**声明——基础设施层在检测到急刹/急加速突变时，以回调方式通知领域层（而非领域层主动调用端口执行检测），与 VO-16/DS-17 描述的"基础设施检测→通知领域"数据流方向一致：
  ```
  func onHardBrakingDetected(event: HardBrakingEvent): Unit
  func onHardAccelerationDetected(event: HardAccelerationEvent): Unit
  ```
...
```
func getReadings(tripId: TripId, window: TimeRange): Result<Array<PhysiologicalSnapshot>, BufferError>
```

- **输入**：`tripId`（当前活跃行程标识，定位边缘侧对应生理数据缓冲）、`window`（TimeRange，碰撞前至碰撞后 ≥10s 的回取时间窗；正常行车过程中窗口中心在碰撞时刻，前后各覆盖至少 5s，可由 DS-06 按需扩展至更宽的窗口）。
- **返回**：成功返回该时间窗内按时序排列的 `Array<PhysiologicalSnapshot>`（VO-03）。
- **错误语义**：`BufferError` 表达可预期的业务情形，至少含：`WindowNotCovered`（请求窗口超出缓冲保留范围，如行程刚开始不足 10s）、`BufferUnavailable`（缓冲未初始化/生理采集组件异常）。DS-06 据此降级——若可得部分快照则以可得数据执行失能判定（降级判定结果标注"数据不完整"），若缓冲完全不可用则无法完成判定并发出退化的 EmergencyActivatedEvent（标注判定置信度降低）。
- **取值范围约束**：`window` 起始不早于碰撞时刻前 10s，终止不早于碰撞时刻后 0s（即窗口至少覆盖碰撞前后 ≥10s），由 DS-06 在调用方确保，端口实现不校验此约束。

**与 VehicleStateBuffer 的对比**：两端口同属领域层依赖接口、均以仓颉 `interface` 声明、均由基础设施层实现并在装配阶段注入 DS-06，区别仅在于：
...
**仓颉语言考量**：与 VehicleStateBuffer 一致，`Result`/`Array`/`TimeRange`/`PhysiologicalSnapshot` 均为前序已确认可行的标准类型构造；`BufferError` 与 VehicleStateBuffer 复用相同错误类型（均为缓冲层问题），无新增语言能力诉求。
...
| 审查意见 | 修改措施 |
|---------|---------|
| **1【一般】DetectionWindow 值对象未在 §3.3 值对象目录中正式定义** | 采纳。§3.3 新增 **VO-20 DetectionWindow** 条目——定义为 `struct` 值对象，明确其概念维度（剩余时长、起始时刻、累计微动观测次数、容差阈值）、协作关系（由 EdgeSessionContext 持有、作为 DS-05 `evaluateLifeDetection` 的输入输出参数）。三栏格式仿照现有值对象。 |
| **2【一般】DomainEvent 类型/接口在设计文档中未定义** | 采纳。§3.5 新增 **§3.5.0 DomainEvent 公共抽象**——以仓颉 `interface`（marker interface）定义，声明三个公共元数据字段：`eventId: String`（事件唯一标识）、`occurredAt: Timestamp`（发生时间戳）、`aggregateId: String`（关联聚合根标识）。各具体事件类型以 `struct` 定义并 `implements DomainEvent`，保证事件载荷不可变性和事件总线编译期类型校验。原 §3.5 事件表标题调整为 §3.5.1 领域事件一览。 |
| **3【轻微】OverrideSignal 和 DriverComprehensiveScore 类型未正式定义** | 采纳方案 (a)。§3.3 新增 **VO-21 OverrideSignal**（`struct`，含 `OverrideType` 枚举——TURNING/BRAKING/ACCELERATING——和时间戳）和 **VO-22 DriverComprehensiveScore**（`struct`，范围 [0,100]，独立于 VO-05 TripScore 的语义，归属 Driver 聚合）。两类型均作为形式化接口（DS-07 `handleOverride`、DriverRepository `updateScore`）的参数类型得到正式定义。 |
| **4【轻微】事件总线契约中的 Type\<E\> 运行时类型标记能力未确认** | 采纳方案 (b) 为主 + 方案 (a) 为优化保留。§3.6 契约 2（同步消费注册）和契约 3（异步消费注册）的 `eventType` 参数从 `Type<E>` 改为 **`eventTypeName: String`**（如 `"RiskDeterminedEvent"`），以字符串表达事件类型判别符，确保事件总线接口契约在仓颉实现时不依赖运行时类型反射能力。同时在各契约中注明"实现阶段若确认仓颉支持运行时类型反射，可优化为类型安全注册 `registerSyncHandler<E: DomainEvent>(eventType: Type<E>, ...)`"。`unregisterSyncHandler` 参数同步改为 `eventTypeName: String`。 |
