...
```
func registerSyncHandler<E: DomainEvent>(eventType: Type<E>, handler: (E) -> Unit)
```
...
- 生命周期：同步 handler 在边缘侧进程生命周期内始终有效，注销通过配套的 `unregisterSyncHandler` 完成。
...
```
func registerAsyncHandler<E: DomainEvent>(eventType: Type<E>, handler: (E) -> Unit): Subscription
```
