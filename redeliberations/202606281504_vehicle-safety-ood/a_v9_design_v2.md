# 车载安全监测系统 领域层 OOD 设计方案（a_v9 / v2）

> 本文档为「智能物联——基于多传感器融合的车载安全监测系统」的架构级 OOD 设计方案，采用 DDD（领域驱动设计）分层思想，聚焦领域层的实体、值对象、聚合根、领域服务和领域事件设计。设计目标语言为仓颉（Cangjie），方案在仓颉类型系统能力范围内做抽象，不涉及具体代码实现。本版（a_v9_v1）在 a_v8_v1 基础上，修复审查确认的 6 个问题：家属手动应急救援联动路径建模、聚合根仓储接口契约定义、远程对讲/视频介入时驾驶员声光提示建模、剩余领域服务形式化方法签名补充、疲劳L2与分心L2共用视线偏离信号的并发处理说明、聚合根-仓储对照表补充，详见文末「修订说明（a_v9_v1）」。
...
| AR-03 Vehicle | VehicleRepository（注） | 按 VehicleId 加载聚合、更新聚合内传感器状态与 OTA 升级状态 |
...
> **注**：VehicleRepository 契约遵循与其他仓储一致的接口模式（findById + save + 乐观锁），因设计文档中 Vehicle 聚合的使用场景较少（传感器自检、OTA 升级状态更新），其方法契约细节在此不单独展开，编码阶段参照 TripRepository / DriverRepository 的接口契约风格实现。
...
- 对于 EmergencyActivatedEvent（碰撞失能）：创建 SafetyAlertEvent（AlertType=COLLISION_DISABILITY）并通过 TripRepository 持久化，不产生独立的 AlertTriggeredEvent（碰撞失能的推送由 EmergencyRescueService 路径处理，不走常规告警通知链路）。
...
- **`aggregateId`**：关联的聚合根标识，标识此事件源于哪个聚合根的状态变更。取值可以是 TripId、DriverId、VehicleId 等聚合根标识的字符串表示。
...
| `EmergencyActivatedEvent` | **EmergencyResponseService（独立事件触发型服务，不经门面汇总）** 判定 BR-06 碰撞+失能条件满足（AlertType=COLLISION_DISABILITY，独占此取值） | Driver 标识、GeoLocation、VehicleStateSnapshot、Timestamp | EmergencyRescueService、PermissionService（自动激活家属端）、**AlertPersistenceService（创建 SafetyAlertEvent）** |
