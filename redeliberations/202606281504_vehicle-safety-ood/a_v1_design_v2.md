...
**职责**：接收四类感知通道的统一输入（SensorReading），按 BR-01~BR-03、分心检出规则和 BR-06 执行融合判定，输出统一的 RiskLevel 和 AlertType。它是边缘侧判定链路的入口——在边缘本地完成轻量判定以保证 500ms 端到端时延和断网可用性。云端承担更重的多维融合行为风控建模，但那是云侧服务而非本领域服务范畴。

**协作**：
- 输入：多条 SensorReading（分别来自 DMS 视觉、生理体征、语音情绪、毫米波雷达、加速度传感器通道）。
...
- 汇总各子判定服务的判定结果，产出 RiskDeterminedEvent（携带 RiskLevel 和 AlertType），驱动干预模块和通知模块。
...
> **设计约束**：LifeDetectionService 存在"有状态"特性（维护判定窗口计时）。在后续详细设计阶段，建议将窗口计时状态封装为独立的 `DetectionWindow` 值对象，使领域服务本身回归纯函数形态，提升可测试性。边缘侧单线程运行环境（见 §6.1）保证了当前设计在当前阶段的可行性。
...
|------|----------|-------------|-----------|
| `RiskDeterminedEvent` | RiskDeterminationService 完成一次融合判定 | Trip 标识、RiskLevel、AlertType、判定时间戳、异常特征快照 | InterventionService（所有类型）、PermissionService（跟踪 L3 持续时长）、PrivacyProtectionService（AlertType=ROAD_RAGE 时触发语音存证录制） |
...
**补充约束**：子判定服务**不得直接调用其他模块的领域服务或直接产出领域事件**——判定结果一律返回给 RiskDeterminationService 门面，由门面统一产出 RiskDeterminedEvent。跨模块协作（如路怒成立触发语音存证、环境调节）由消费方订阅 RiskDeterminedEvent 完成。这保证了模块间的松耦合和依赖方向的单向性。
...
DESIGN_WRITTEN:D:\软件测试\redeliberations\202606281504_vehicle-safety-ood\a_v1_design_v2.md
