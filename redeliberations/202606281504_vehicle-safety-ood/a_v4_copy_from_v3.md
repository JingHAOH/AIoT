# 车载安全监测系统 领域层 OOD 设计方案（a_v4 / v1）

> 本文档为「智能物联——基于多传感器融合的车载安全监测系统」的架构级 OOD 设计方案，采用 DDD（领域驱动设计）分层思想，聚焦领域层的实体、值对象、聚合根、领域服务和领域事件设计。设计目标语言为仓颉（Cangjie），方案在仓颉类型系统能力范围内做抽象，不涉及具体代码实现。本版（a_v4_v1）在 a_v3_v1 基础上，修复诊断确认的 6 个问题：DS-05 熄火落锁事件建模缺失、DS-08 L3 计时状态归属未明确、摄像头物理遮挡检测源与恢复路径缺失、DriverScoreUpdatedEvent 消费方建模违反 DDD 惯例、VO-13 生命体征摘要数据来源悬空、DS-16 跨端推送中转路径缺失，详见文末「修订说明（a_v4_v1）」。
