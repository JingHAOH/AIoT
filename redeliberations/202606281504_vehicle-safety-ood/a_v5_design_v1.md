# 车载安全监测系统 领域层 OOD 设计方案（a_v5 / v1）

> 本文档为「智能物联——基于多传感器融合的车载安全监测系统」的架构级 OOD 设计方案，采用 DDD（领域驱动设计）分层思想，聚焦领域层的实体、值对象、聚合根、领域服务和领域事件设计。设计目标语言为仓颉（Cangjie），方案在仓颉类型系统能力范围内做抽象，不涉及具体代码实现。本版（a_v5_v1）在 a_v4_v1 基础上，修复诊断确认的 8 个问题：§3.5 章节结构错误与编号重复、Trip:Driver=1:1 基数约束内部矛盾、领域事件总线契约缺失、会话上下文未正式建模与 L3DurationTracker 双重归属张力、外部系统端口契约缺失、急刹/急加速阈值来源未定义、救援机构角色未建模、家属通知偏好未形式化，详见文末「修订说明（a_v5_v1）」。
...
- **领域层通过端口参数声明阈值需求**：DrivingBehaviorTrackingPort 的方法契约中显式包含阈值参数：
  ```
  func detectHardBraking(signal: AccelerationSignal, threshold: DecelerationThreshold): Option<HardBrakingEvent>
  func detectHardAcceleration(signal: AccelerationSignal, threshold: AccelerationThreshold): Option<HardAccelerationEvent>
  ```
  `DecelerationThreshold` 和 `AccelerationThreshold` 为值对象，由领域层定义其数值范围约束（如"减速度阈值必须为正实数"），具体数值由基础设施层按车型/传感器标定后配置注入。验收测试时固定阈值以确保可复现性。
