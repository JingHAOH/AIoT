# 车载安全监测系统 领域层 OOD 设计方案（a_v3 / v1）

> 本文档为「智能物联——基于多传感器融合的车载安全监测系统」的架构级 OOD 设计方案，采用 DDD（领域驱动设计）分层思想，聚焦领域层的实体、值对象、聚合根、领域服务和领域事件设计。设计目标语言为仓颉（Cangjie），方案在仓颉类型系统能力范围内做抽象，不涉及具体代码实现。本版（a_v3_v1）在 a_v2_v1 基础上，修复诊断确认的 4 个问题：BR-05 急刹/急加速全链路数据源缺失、二次身份验证授权流程未建模、后排红外摄像头感知源遗漏、Driver 综合风险评分写回链路悬空，详见文末「修订说明（a_v3_v1）」。
...
- 由**基础设施层的持续加速度监测组件**在整个行程中持续检测加速度信号——当加速度突变超过阈值（急刹：减速度 > 阈值；急加速：加速度 > 阈值）时，通过领域层声明的端口（**DrivingBehaviorTrackingPort**，见决策 17）通知领域层更新计数器。
...
- **写回路径**：ScoringService 完成 Driver 级评分计算后，通过两种方式之一将结果写回 Driver 聚合：（a）产出 **DriverScoreUpdatedEvent**（领域事件），由 Driver 聚合或其仓储消费并更新综合风险评分字段；（b）ScoringService 直接通过 DriverRepository 更新 Driver 聚合的评分字段（因评分是跨聚合的只读统计，在此场景下跨聚合直接写入可接受，由乐观锁保证一致性）。推荐方案 (a) 以保持事件驱动的解耦风格——ScoringService 产出 DriverScoreUpdatedEvent，Driver 聚合的仓储订阅该事件并执行字段更新。
...
| `DriverScoreUpdatedEvent` | ScoringService 完成 Driver 级综合风险评分计算并写回 Driver 聚合 | Driver 标识、新评分、旧评分、计算周期 | Driver 聚合（更新综合风险评分字段） |
...
DESIGN_WRITTEN:D:\软件测试\redeliberations\202606281504_vehicle-safety-ood\a_v3_design_v1.md
