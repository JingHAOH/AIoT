# 车载安全监测系统 领域层 OOD 设计方案（a_v8 / v1）

> 本文档为「智能物联——基于多传感器融合的车载安全监测系统」的架构级 OOD 设计方案，采用 DDD（领域驱动设计）分层思想，聚焦领域层的实体、值对象、聚合根、领域服务和领域事件设计。设计目标语言为仓颉（Cangjie），方案在仓颉类型系统能力范围内做抽象，不涉及具体代码实现。本版（a_v8_v1）在 a_v7_v1 基础上，修复诊断确认的 7 个问题：场景8步骤编号重复、DS-10/DS-11缺少形式化接口定义、AR-02评分写回路径残留歧义表述、E-02编号空缺、DS-14遮挡检测端口缺失、路怒计数口径说明缺失、DS-11累计次数未穷举，详见文末「修订说明（a_v8_v1）」。
...
| `FamilyAccessGrantedEvent` | 家属获得对某 Driver 的访问权限（常规 60s 或自动激活、或物理遮挡解除恢复） | SystemAccount 标识、Driver 标识、Permission、授权原因（常规/高危/遮挡恢复） | 家属 APP 推送、远程对讲/视频通道建立 |
